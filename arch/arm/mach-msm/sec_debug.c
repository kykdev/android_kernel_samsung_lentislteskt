...
static void __iomem *restart_reason=NULL;
...
int sec_debug_subsys_init(void)
{
	...
	pr_info("%s: SMEM_ID_VENDOR2=%d size=%d\n",
		__func__,  SMEM_ID_VENDOR2, sizeof(struct sec_debug_subsys));
	...
	secdbg_subsys->krait = (struct sec_debug_subsys_data_krait *)
	    (unsigned int)(smem_virt_to_phys(&secdbg_subsys->priv.krait)&0xFFFFFFFF);
	secdbg_subsys->rpm = (struct sec_debug_subsys_data *)
		(unsigned int)(smem_virt_to_phys(&secdbg_subsys->priv.rpm)&0xFFFFFFFF);
	secdbg_subsys->modem = (struct sec_debug_subsys_data_modem *)
		(unsigned int)(smem_virt_to_phys(&secdbg_subsys->priv.modem)&0xFFFFFFFF);	
	secdbg_subsys->dsps = (struct sec_debug_subsys_data *)
		(unsigned int)(smem_virt_to_phys(&secdbg_subsys->priv.dsps)&0xFFFFFFFF);
...
static int __init sec_dt_addr_init(void)
{
	...
	pr_emerg("%s: restart_reason addr : 0x%x(0x%x)\n", __func__,
		(unsigned int)restart_reason,(unsigned int)virt_to_phys(restart_reason));
	...
	pr_emerg("%s: upload_cause addr : 0x%x(0x%x)\n", __func__,
		(unsigned int)upload_cause,(unsigned int)virt_to_phys(upload_cause));
